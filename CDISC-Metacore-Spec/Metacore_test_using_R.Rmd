---
title: "Example file from Metacore - Updated to use Define.xml file from CDISC Pilot Data"
...
```{r}
doc <- read_xml("/Users/Pinnacle 21 Community/SDTM-MSG_v2/m5/datasets/cdiscpilot01/tabulations/sdtm/define.xml") #Update path to define.xml location
xml_ns_strip(doc)
```
...
value_spec <- value_spec %>%
  mutate(across(where(is.character), ~ifelse(is.na(.), "test", .)))
...
subset <- test %>% select_dataset("IE")
subset$ds_spec

# a simplified dataframe 
subset_t <- test %>% select_dataset("IE", simplify = TRUE)
```
